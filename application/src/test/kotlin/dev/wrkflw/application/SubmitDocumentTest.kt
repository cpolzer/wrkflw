...
class SubmitDocumentTest {
    ...
    @Test
    fun `returns DefinitionNotFound when definition does not exist`() =
        runTest {
            val result =
                service.execute(
    ...
    @Test
    fun `returns Unauthorized when actor is not in initiator group`() =
        runTest {
            val result =
                service.execute(
                    SubmitDocumentCommand(
                        "document-approval",
                        "doc-1",
                        ActorContext(ActorId("user1"), setOf(reviewerGroup)),
                    ),
                )
    ...
    @Test
    fun `saves flow instance and starts workflow on success`() =
        runTest {
    ...
    @Test
    fun `writes FLOW_STARTED audit entry on success`() =
        runTest {
            val actor = ActorContext(ActorId("author1"), setOf(authorGroup))
