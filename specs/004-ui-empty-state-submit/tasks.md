...
- [ ] T006 [US1] Add imports to `ui/src/views/MySubmissionsView.vue`: `useAuthStore` from `@/stores/auth`, `AVAILABLE_DEFINITIONS` from `@/api/definitions`, `OnyxButton` from `sit-onyx`
- [ ] T007 [US1] Add computed values to `<script setup>` in `ui/src/views/MySubmissionsView.vue`: `auth = useAuthStore()`, `firstDefinition = computed(() => AVAILABLE_DEFINITIONS[0])`, `canSubmit = computed(() => !!firstDefinition.value && auth.isInGroup(firstDefinition.value.initiatorGroup))`
- [ ] T008 [US1] Replace `<h1>My Submissions</h1>` with a `<header class="submissions-view__header">` flex container containing the `<h1>` and a conditional `<OnyxButton v-if="canSubmit" label="Submit new document" color="primary" :link="{ name: 'submit-flow', params: { definitionId: firstDefinition!.definitionId } }" />` in `ui/src/views/MySubmissionsView.vue`
- [ ] T009 [US1] Add `.submissions-view__header` CSS rule (`display: flex; align-items: center; justify-content: space-between; margin-bottom: 1.5rem;`) and remove the existing `h1` margin rule in `ui/src/views/MySubmissionsView.vue`
...
- [ ] T010 [US2] Write test: initiator with 0 submissions → `OnyxEmpty` rendered with headline text containing "not submitted" and a secondary `OnyxButton` in `ui/tests/unit/MySubmissionsView.spec.ts`
- [ ] T011 [US2] Write test: no definitions available → `OnyxEmpty` rendered, no `OnyxButton`, description contains "no submission types" (FR-007) in `ui/tests/unit/MySubmissionsView.spec.ts`
- [ ] T012 [US2] Write test: non-initiator with 0 submissions → `OnyxEmpty` rendered, no `OnyxButton`, description contains "permission" in `ui/tests/unit/MySubmissionsView.spec.ts`

### Implementation for User Story 2

- [ ] T013 [US2] Add `OnyxEmpty` to the imports from `sit-onyx` in `ui/src/views/MySubmissionsView.vue`
- [ ] T014 [US2] Replace `<p v-else class="submissions-view__empty">No submissions yet.</p>` with `<OnyxEmpty v-else>` block in `ui/src/views/MySubmissionsView.vue`: default slot text "You have not submitted any documents for approval yet.", `#description` slot with three conditional `<span>` branches (canSubmit / no definitions / no permission), `#buttons` slot with conditional `OnyxButton` (same props as header CTA, only when `canSubmit`)
- [ ] T015 [US2] Remove the now-unused `.submissions-view__empty` CSS rule from `ui/src/views/MySubmissionsView.vue`
