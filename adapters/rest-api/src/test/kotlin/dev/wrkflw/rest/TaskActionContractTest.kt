...
class TaskActionContractTest {
    ...
    @Test
    fun `POST claim returns 200 with TaskSummary on success`() =
        withApp(
            claimTask = ClaimTaskUseCase { ClaimTaskResult.Success(sampleTask) },
        ) { client ->
            val response =
                client.post("/api/v1/tasks/$taskId/claim") {
    ...
    @Test
    fun `POST claim returns 404 when task not found`() =
        withApp(
    ...
    @Test
    fun `POST claim returns 403 when actor not in candidate group`() =
        withApp(
    ...
    @Test
    fun `POST claim returns 409 when task not claimable`() =
        withApp(
    ...
    @Test
    fun `POST claim returns 400 when actor header is missing`() =
        withApp { client ->
    ...
    @Test
    fun `POST release returns 200 with TaskSummary on success`() =
        withApp(
            releaseTask =
                ReleaseTaskUseCase {
                    ReleaseTaskResult.Success(sampleTask.copy(status = TaskStatus.PENDING, ownerId = null))
                },
        ) { client ->
            val response =
                client.post("/api/v1/tasks/$taskId/release") {
                    header("X-Actor-Id", "reviewer1")
                    header("X-Actor-Groups", "reviewers")
                }
            response.status shouldBe HttpStatusCode.OK
    ...
    @Test
    fun `POST release returns 403 when actor is not owner`() =
        withApp(
    ...
    @Test
    fun `POST release returns 409 when task not releasable`() =
        withApp(
    ...
    @Test
    fun `POST decision returns 200 with FlowStatus on success`() =
        withApp(
            submitDecision = SubmitDecisionUseCase { SubmitDecisionResult.Success(sampleInstance) },
        ) { client ->
            val response =
                client.post("/api/v1/tasks/$taskId/decision") {
                    header("X-Actor-Id", "reviewer1")
                    header("X-Actor-Groups", "reviewers")
    ...
    @Test
    fun `POST decision returns 403 when actor is not owner`() =
        withApp(
    ...
    @Test
    fun `POST decision returns 409 when task is not decidable`() =
        withApp(
    ...
    @Test
    fun `POST decision returns 422 when outcome is invalid`() =
        withApp { client ->
            val response =
                client.post("/api/v1/tasks/$taskId/decision") {
                    header("X-Actor-Id", "reviewer1")
    ...
    @Test
    fun `POST decision returns 404 when task not found`() =
        withApp(
